...
### 🎨 2025-01-10 (难度选择UI优化)
- **🆕 UI改进**: 将难度选择从复杂的按钮网格优化为简洁的Picker下拉菜单
- **📱 空间优化**: 
  - iPad横屏：左侧面板空间利用更高效，节省约60%垂直空间
  - 默认布局：从6行按钮减少为1个紧凑选择器
- **🎯 用户体验提升**: 
  - 原生iOS下拉菜单体验，符合用户使用习惯
  - 更好的可访问性支持和VoiceOver兼容
  - 统一的选择器交互模式，与时间设置保持一致
- **🔧 代码简化**: 移除复杂的网格布局逻辑，减少约80行代码，使用标准Picker组件
- **🌐 保持兼容**: 完全保留现有的本地化支持和功能特性，无功能损失
